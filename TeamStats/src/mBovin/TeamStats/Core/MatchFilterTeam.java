...
public class MatchFilterTeam implements MatchFilter {

	@Override
	public boolean filter(Match match, Object[] args) {
		Integer teamIndex = (Integer) args[0];
		Boolean includeHome = (Boolean) args[1];
		Boolean includeAway = (Boolean) args[2];
		
		return ((match.getmHomeTeamId() == teamIndex) && includeHome) ||
				((match.getmAwayTeamId() == teamIndex) && includeAway);
	}
